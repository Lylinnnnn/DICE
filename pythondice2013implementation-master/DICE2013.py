...
path = 'D:/ipopt/ipopt.exe'  # ipopt执行文件
...
def diceModel2013(**kwargs):
    ...
    ###SETS AND INDICES###
    model.time_periods = pe.Set(initialize=list(kwargs['rr'].keys()))
    ...
    model.TATM = pe.Var(model.time_periods, domain=pe.NonNegativeReals,
                        bounds=tatmBounds)  # Increase temperature of atmosphere (degrees C from 1900)
    model.TOCEAN = pe.Var(model.time_periods, domain=pe.Reals,
                          bounds=toceanBounds)  # Increase temperatureof lower oceans (degrees C from 1900)
    model.MAT = pe.Var(model.time_periods, domain=pe.NonNegativeReals,
                       bounds=matBounds)  # Carbon concentration increase in atmosphere (GtC from 1750)
    model.MU = pe.Var(model.time_periods, domain=pe.NonNegativeReals,
                      bounds=muBounds)  # Carbon concentration increase in shallow oceans (GtC from 1750)
    model.ML = pe.Var(model.time_periods, domain=pe.NonNegativeReals,
                      bounds=mlBounds)  # Carbon concentration increase in lower oceans (GtC from 1750)
    ...
    model.C = pe.Var(model.time_periods, domain=pe.NonNegativeReals,
                     bounds=cBounds)  # Consumption (trillions 2005 US dollars per year)
    model.K = pe.Var(model.time_periods, domain=pe.NonNegativeReals,
                     bounds=kBounds)  # Capital stock (trillions 2005 US dollars)
    model.CPC = pe.Var(model.time_periods, domain=pe.NonNegativeReals,
                       bounds=cpcBounds)  # Per capita consumption (thousands 2005 USD per year)
    model.I = pe.Var(model.time_periods, domain=pe.NonNegativeReals)  # Investment (trillions 2005 USD per year)
    model.S = pe.Var(model.time_periods, domain=pe.Reals,
                     bounds=sBounds)  # Gross savings rate as fraction of gross world product
    model.RI = pe.Var(model.time_periods, domain=pe.Reals)  # Real interest rate (per annum)
    model.Y = pe.Var(model.time_periods,
                     domain=pe.NonNegativeReals)  # Gross world product net of abatement and damages (trillions 2005 USD per year)
    model.YGROSS = pe.Var(model.time_periods,
                          domain=pe.NonNegativeReals)  # Gross world product GROSS of abatement and damages (trillions 2005 USD per year)
    model.YNET = pe.Var(model.time_periods,
                        domain=pe.Reals)  # Output net of damages equation (trillions 2005 USD per year)
    ...
    model.ABATECOST = pe.Var(model.time_periods,
                             domain=pe.Reals)  # Cost of emissions reductions  (trillions 2005 USD per year)
    model.MCABATE = pe.Var(model.time_periods, domain=pe.Reals)  # Marginal cost of abatement (2005$ per ton CO2)
    model.CCA = pe.Var(model.time_periods, domain=pe.Reals,
                       bounds=ccaBounds)  # Cumulative industrial carbon emissions (GTC)
    ...
    def radiativeForcing(model, t):
        return (model.FORC[t] == kwargs['fco22x'] * (pe.log10(model.MAT[t] / 588.000) / pe.log10(2)) +
                kwargs['forcoth'][t])
    ...
    def abatementCost(model, t):
        return (model.ABATECOST[t] == model.YGROSS[t] * kwargs['cost1'][t] * (model.MIU[t] ** kwargs['expcost2']) * (
                    kwargs['partfract'][t] ** (1 - kwargs['expcost2'])))

    model.abatementCost = pe.Constraint(model.time_periods, rule=abatementCost)

    def mcAbatement(model,
                    t):  # constraint is giving an error.  solver or pyomo may not like exponents less than two.
    ...
    def carbonPriceEq(model,
                      t):  # constraint is giving an error.  solver or pyomo may not like fractional exponents less than two
        myExp = kwargs['expcost2'] - 1
        if myExp >= 2:
            return (model.CPRICE[t] == kwargs['pbacktime'][t] * (model.MIU[t] / kwargs['partfract'][t]) ** myExp)
        else:
            myExp += 1
            return (model.CPRICE[t] * model.MIU[t] / kwargs['partfract'][t] == kwargs['pbacktime'][t] * (
                        model.MIU[t] / kwargs['partfract'][t]) ** myExp)
    ...
    def atmosphericConcentration(model, t):
        if t == 1:
            ...
            return (model.MAT[t] == model.MAT[t - 1] * kwargs['b11'] + model.MU[t - 1] * kwargs['b21'] + model.E[
                t - 1] * kwargs['tstep'] / 3.666)
    ...
    def upperOceanConcentration(model, t):
        if t == 1:
            ...
            return (model.MU[t] == model.ML[t - 1] * kwargs['b32'] + model.MU[t - 1] * kwargs['b22'] + model.MAT[
                t - 1] * kwargs['b12'])
    ...
    def atmosphericTemperature(model, t):
        if t == 1:
            ...
            return (model.TATM[t] == model.TATM[t - 1] + kwargs['c1'] * (
    ...
    def capitalBalance(model, t):
        if t == 1:
            ...
            return (model.K[t] == (1 - kwargs['dk']) ** kwargs['tstep'] * model.K[t - 1] + kwargs['tstep'] * model.I[
                t - 1])
    ...
    def interestRateEqn(model, t):
        if t == 1:
            ...
            return (model.RI[t] == (1 + kwargs['prstp']) * (model.CPC[t] / model.CPC[t - 1]) ** (
                        kwargs['elasmu'] / kwargs['tstep']) - 1)
    ...
    def instUtilityEqn(model, t):
        return (model.PERIODU[t] == ((model.C[t] * 1000 / kwargs['l'][t]) ** (1 - kwargs['elasmu']) - 1) / (
                    1 - kwargs['elasmu']) - 1)
...
*Utility
 util..               UTILITY        =E= tstep * scale1 * sum(t,  CEMUTOTPER(t)) + scale2 ;
    ...
    solver = pyomo.opt.SolverFactory(
        'ipopt', executable=path)  # options are 'couenne', 'ipopt', and 'bonmin'.  ipopt is continuous and bonmin uses mixed integer
...
if __name__ == "__main__":
    ...
    params['gsig'] = getCumulativeEfficiencyImprovement(params['gsigma1'], params['dsig'], params['tstep'],
                                                        params['numPeriods'])
    ...
    params['cost1'] = getAdjustedCostForBackstop(params['pbacktime'], params['sigma'], params['expcost2'],
                                                 params['numPeriods'])
    ...
    params['partfract'] = getFractionOfEmmissionsInControlRegime(params['periodfullpart'], params['partfractfull'],
                                                                 params['partfract2010'], params['numPeriods'])
